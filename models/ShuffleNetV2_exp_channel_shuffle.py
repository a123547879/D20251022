...
class ShuffleV2Block(nn.Module):
    ...
    def __init__(self, inp, oup, mid_ch, *, ksize, stride):
        ...
        if stride == 2:
            ...
            self.alternativeShuffle = nn.Conv2d(inp * 2, inp * 2, kernel_size= 1, groups= 2, bias= False)
    ...
    def forward(self, old_x):
        if self.stride == 1:
            shuffled = self.alternativeShuffle(old_x)

            batch_size, n_channels, h, w = shuffled.size()
            if n_channels % 2 != 0:
                raise ValueError('f"Channels must be even, got {num_channels}"')
            
            split_size = n_channels // 2
            x_proj = shuffled[:, :split_size, :, :]
            x = shuffled[:, split_size:, :, :]
